...
#define DECLINATION 3.56 // Declination (degrees) in Chicago, IL
...
void setup(){
  ...
  Serial1.begin(9600, SERIAL_8N1, 16, 17);
	...
	GPSLogFile.setMaxFileSize(100000);
	GPSLogFile.setWriteBufferSize(90);
	GPSLogFile.startLog("logFiles", "GPS_");

	IMULogFile.setMaxFileSize(100000);
	IMULogFile.setWriteBufferSize(90);
...
void loop(){
		...
		case 1:
			if(Serial1.available())
			{
				...
				if(c == '$')
				{
					...
					{
						// millis as a sync time for the two sensors
    ...
    int timestamp = millis();
		...
		sprintf(csvBuffer, "%d, %4.3f,%4.3f,%4.3f,%4.3f,%4.3f,%4.3f,%4.3f,%4.3f,%4.3f,", timestamp, data1, data2, data3, data4, data5, data6, data7, data8, data9);//, roll, pitch, heading);
		//Output data to terminal for debug
