...
FIL countFile;										  /* File that records the sequence of track files*/
...
uint8_t DATALOG_SD_Log_Enable(void)
{
  ...
  if(f_open(&countFile, "sequence_number.txt", FA_OPEN_EXISTING | FA_WRITE | FA_READ) == FR_OK){
	  if(f_read(&countFile, &sdcard_file_counter, sizeof(uint16_t), &byteswritten) == FR_OK){
		  sdcard_file_counter+=1;
	  }
	  f_close(&countFile);
  }
  if(f_open(&countFile, "sequence_number.txt", FA_CREATE_ALWAYS | FA_WRITE | FA_READ) != FR_OK)
	  return 0;
  f_write(&countFile, (const void*)&sdcard_file_counter, sizeof(uint16_t), &byteswritten);
